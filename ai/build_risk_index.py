...
# 표본부족 판정 기준 (점포수). 모델 학습 필터(train_model.py CELL_MIN_STORES=30)와 값을 맞춘다.
#
# 이력.
#   30 -> 50 상향(2026-08-18). 당시 근거는 "최신분기 단일 스냅샷" 기준 민감도였다.
#   50 -> 30 환원(2026-08-29). 아래 두 가지 때문이다.
#
#   (1) 근거 재현 실패. 위 상향 근거였던 "30에서 상위 10개 리프트 1.14배"가 재현되지 않는다.
#       ai/validate_ranking.py로 기준선만 바꿔가며 다시 재면(모델 단독 정렬, 기준분기
#       20243·20244 평균, 미래 4분기 실측 대비):
#
#           기준   셀수   스피어만   리프트(상위10%)
#           50     232    0.324      1.180
#           40     295    0.318      1.248
...
#           30     384    0.349      1.291
#
#       두 지표 모두 30이 50보다 낫다. 50을 지지하던 정확도 우위가 실측에서 사라졌다.
#       단, 기준분기가 2개뿐이라 0.324 vs 0.349는 오차 범위로 봐야 한다. 이 표의 주장은
#       "30이 더 정확하다"가 아니라 "30이 덜 정확하다는 증거가 없다"이다.
#       커버리지가 넓어지는 쪽을 고른 근거가 이것이다.
#
#       새로 들어온 소표본 셀(점포 30~49)이 순위를 망치는지도 분리해서 쟀다.
#           20243  기존셀(>=50) n=213 스피어만 0.347 / 리프트 1.235
#                  신규셀(30~49) n=148 스피어만 0.420 / 리프트 1.621
#           20244  기존셀(>=50) n=251 스피어만 0.300 / 리프트 1.124
#                  신규셀(30~49) n=156 스피어만 0.284 / 리프트 1.427
#       신규 셀에서 리프트가 오히려 높다. 모델이 작은 셀을 못 맞히는 것이 아니다.
#
#       [혼동 주의 — 0.4184와 비교하지 말 것]
#       train_model.py:39의 "스피어만 0.4184 / 리프트 1.433"은 이 값과 무관하다.
#       그쪽은 고정 분할(TRAIN_END/VALID_END=2024Q2) 1회 측정한 학습 검증 성능이고,
#       필터도 CELL_MIN_STORES=30이라 SAMPLE_MIN을 바꿔도 한 자리도 변하지 않는다.
#       여기 표는 validate_ranking.py가 미래 4분기 실측 대비로 잰 값이다. 측정 대상도
#       분할도 다르다. 두 숫자를 빼서 "성능이 떨어졌다"고 읽으면 틀린다.
#
#   (2) 학습-조회 기준 불일치. 모델은 CELL_MIN_STORES=30으로 학습·검증됐는데 화면만 50으로
#       잘라 보여주고 있었다. 검증한 범위보다 좁게 노출하는 쪽이 오히려 방어하기 어렵다.
#
# 커버리지 영향(최신분기 20254, 전체 1,802셀 / 29개 읍면동 / 74개 업종 기준):
#   판정 셀      231 -> 382개
#   점포 커버율  61.8% -> 74.8%
#   읍면동       27 -> 29개(전 지역)
#   판정 업종    33 -> 46개 (전역 미판정 업종 41 -> 28개)
#   업종 평균 커버율 23.3% -> 36.1%
#
# 등급·지도 영향(같은 기준분기).
#   등급 기준선  위험 10.35% -> 9.71% / 주의 7.26% -> 6.90% (분위수 기준이라 자동 이동)
#   위험 셀      24 -> 39개. 순증이며 등급이 내려가 빠지는 셀은 0개다.
#   위험 셀 업종 편중  교육계열 18/24(75%) -> 22/39(56%)로 완화. 다만 여전히 과반이다.
#   동당 판정 업종 수  중위 6개 -> 10개 ("표본 충분 업종이 N개뿐" 안내 문구의 직접 개선)
#
# 기준선을 내려도 안 풀리는 것. 순위표 상위권은 거의 그대로다(상위 12개 중 바뀌는 건
# 향남읍 욕탕·신체관리 하나). 교육 업종이 상위를 점거하는 것은 표본 문제가 아니라
# 실제로 폐업률이 높기 때문이다. 업종 편중은 기준선이 아니라 화면(업종 필터·정규화)에서
# 다뤄야 한다.
#
# 대가. 셀당 표본이 작아지므로 Wilson 신뢰구간 폭이 평균 5.4%p -> 7.2%p로 넓어진다.
# 이 불확실성은 숨기지 않고 화면에 구간으로 함께 표기한다(정렬은 계속 신뢰하한 기준).
# 특히 조기경보 상위 10% 목록은 절반가량이 점포 50곳 미만 셀로 바뀌므로(20243 36개 중
# 18개, 20244 41개 중 18개) 목록 화면에 구간 표기가 필수다.
SAMPLE_MIN = 30
