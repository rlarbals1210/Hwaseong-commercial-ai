...
def main():
    ...
    if sample_ok.any():
        ...
        model_rank = subset["_예측폐업률_내부용"].rank(ascending=False, pct=True)
        observed_rank = subset["위험도_하한_pct"].rank(ascending=False, pct=True)
        ensemble = (model_rank + observed_rank) / 2
        ...
        merged.loc[sample_ok, "우선순위"] = ensemble.rank(method="first").astype(int)
