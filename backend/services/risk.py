...
def action_message(level: str, anomaly: bool) -> str:
    ...
    쓰지 않는다. 모든 문구는 공무원의 다음 행동(검토·관찰)을 제안하는 형태로만 쓴다.

    "현장 확인"이라는 말은 쓰지 않는다 — 담당 부서 면담(2026-09) 결과 현장 방문은 거의
    이뤄지지 않으며, 실제로 필요한 것은 지원 기준을 정할 때 쓸 근거였다.
    """
    if anomaly:
        return "트렌드 이상 — 우선 검토"
    if level == "표본부족":
        return "표본 부족 — 통계 판단 보류, 개별 확인 필요"
    if level == "위험":
        return "지원 검토 우선순위 높음"
    if level == "주의":
        return "분기별 모니터링 강화"
    return "정기 관찰 유지"
