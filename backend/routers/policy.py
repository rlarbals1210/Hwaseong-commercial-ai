...
def get_inspection_priority(
    ...
    """지원 검토 우선순위 — 실제 관측 폐업률(x축) × 영향 점포 수(y축) 4사분면.

    이 API는 정책자금 배분 대상을 결정하지 않는다. 담당자가 '어디부터 검토할지'
    순서를 좁히는 보조 자료이며, 최종 판단과 지원 결정은 공무원이 한다.

    담당 구역만 보는 담당자가 많아 dong(읍면동)으로도 좁힐 수 있게 했다. 두 필터는
    AND로 걸린다 — 조기경보(alerts.py)의 읍면동 필터와 같은 동작이다.
    x축은 예측값이 아니라 실제 관측 폐업률(4분기 누적)이고, 표본부족 셀은 제외한다
    (기준은 risk_thresholds.json의 sample_min. 2026-08-25 현재 50).
