...
- **선택 상권 → 입지·인근 비교**: 공개 GeoJSON에서 경계선을 공유하는 인접 지역 중 같은 업종의 표본이 충분한 후보를 최대 3곳 제시합니다. 시 전체에서 계산한 점수·순위를 그대로 사용하고 두 지역 비교로 연결합니다. 중심점 거리·이동시간 순위가 아닙니다.
- **선택 상권 → 방문·검색 패턴**: 업종과 무관한 읍면동 전체의 요일별 유동 패턴과 업종 대표 검색어의 전국 검색 관심도를 구분해 표시합니다.
- **선택 상권 → 창업비용**: 견적을 만원 단위로 입력해 초기 자금·월 고정비·손익분기 매출·가정 매출의 월 영업수지를 계산합니다. 보증금과 예비자금은 초기 지출과 구분하며, 면적 단위 변경 시 자동 환산합니다. 임대료·매출 자동 추정이나 투자금 회수기간 예측은 하지 않습니다.
