...
- **읍면동 필터**: 맞춤 추천 패널 상단에서 지역 이름을 검색하고 여러 읍면동을 선택해 추천 카드·지역 목록의 범위를 좁힙니다. 기본값과 선택 전체 해제는 화성시 전체입니다. 점수·순위는 화성시 전체 기준을 유지하며, 추천 근거가 부족한 지역만 남으면 다른 지역으로 대체하지 않고 추천 보류를 표시합니다. 지도는 선택 범위를 강조합니다. 지역·업종 직접 조회와 별도 두 지역 비교는 이 필터의 영향을 받지 않습니다.
- **지역부터 찾기**: 읍면동을 고르면 기존 공통 적합도 점수로 비교한 업종 5개를 보여줍니다. 업종 사이의 절대 성공률이 아니므로 업종 탐색 목록에는 등급을 표시하지 않습니다.
- **적합도 확인**: 원하는 읍면동과 업종을 함께 정한 뒤 적합도와 상세 분석을 바로 엽니다. 업종을 바꿔도 선택 지역이 유지됩니다.
...
- **대시보드(`/dashboard`)·지원 검토(`/policy`)**: 읍면동과 업종 드롭다운을 펼쳐 이름으로 검색하고 한 항목을 선택합니다. 각각 전체로 초기화할 수 있으며 기존 조회 기준을 유지합니다.
- **상권 위험 지도(`/map`)**: 지역 검색에서 읍면동을 선택하면 해당 경계로 이동하고 상세 패널을 엽니다. `화성시 전체로 초기화`로 전체 지도에 돌아갑니다.
- **상권 비교(`/compare`)**: 기준·비교 상권의 선택창에서 읍면동과 업종을 검색합니다. 선택은 `선택 적용`을 눌렀을 때 함께 반영되고, 취소·닫기·Esc는 기존 상권을 유지합니다. 지역을 변경했을 때 기존 업종이 해당 지역에 없으면 업종을 다시 선택해야 합니다. 추천 후보를 눌러도 적용 전까지 창을 유지합니다.

공개 둘러보기의 청록색 접기/펼치기 형식을 `SearchableSelect`로 공유합니다. 공무원 화면은 기존 API에 맞춰 단일 선택을 사용하며, 비교창은 `CellPickerDialog`에서 임시 선택값을 관리합니다.
