...
- **읍면동 필터**: 업종부터 찾기와 추천 패널에서 지역 이름을 검색하고 여러 읍면동을 선택해 추천 카드·지역 목록의 범위를 좁힙니다. 기본값과 선택 전체 해제는 화성시 전체입니다. 점수·순위는 화성시 전체 기준을 유지하며, 추천 근거가 부족한 지역만 남으면 다른 지역으로 대체하지 않고 추천 보류를 표시합니다. 지도는 선택 범위를 강조합니다. 지역·업종 직접 조회와 별도 두 지역 비교는 이 필터의 영향을 받지 않습니다.
