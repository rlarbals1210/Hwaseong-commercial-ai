...
4. **현장점검 우선순위** — 실제 관측 폐업률(x) × 영향 점포 수(y) 4분면 (PolicyPage)
   ...
   - **2026-08-26 확정 — x축은 등급(위험 = 화성시 상위 10%)이다.** `high_risk = (risk_grade == "위험")`.
     ...
     ① 중위값을 쓰면 표본충분 231셀의 절반인 ~115셀이 "우선 확인"이 되어 우선순위 화면의 기능을
     잃는다(등급 기준은 24셀 — 한 분기에 실제로 돌 수 있는 규모). ② 조기경보·지도·셀 상세가 전부
     등급을 말하는데 여기만 다른 기준이면 "네 화면이 같은 숫자를 말한다"는 원칙과 어긋난다.
     ③ "왜 24곳입니까" → "상위 10%입니다"가 "중위값 위입니다"보다 방어하기 쉽다.
   - 중위값으로 바꾼 원래 이유였던 "주의 46셀이 하위 사분면에서 안전해 보인다"는 다른 방법으로
...
| 사분면 x축 | **등급(위험 = 상위 10%)**. 중위값으로 바꾸지 말 것 — 231셀의 절반이 "우선 확인"이 되어 우선순위가 아니게 된다(2026-08-26 확정, MVP 4번 항목 참조) |
